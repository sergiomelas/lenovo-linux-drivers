...
scripts/config --set-val CONFIG_CC_IS_CLANG y                         # Force acknowledge Clang toolchain presence[cite: 13]
scripts/config --enable  CONFIG_ARCH_SUPPORTS_LTO_CLANG               # Validate architecture LTO capabilities[cite: 13]
scripts/config --enable  CONFIG_ARCH_SUPPORTS_LTO_CLANG_THIN          # Validate ThinLTO specific architecture support[cite: 13]
scripts/config --enable  CONFIG_HAS_LTO_CLANG                         # Confirm toolchain LTO readiness[cite: 13]
scripts/config --disable CONFIG_LTO_NONE                              # Turn off 'No LTO' choice[cite: 13]
scripts/config --disable CONFIG_LTO_CLANG_FULL                        # Turn off Full LTO choice[cite: 13]
scripts/config --enable  CONFIG_LTO_CLANG_THIN                        # Force enable ThinLTO choice[cite: 13]
scripts/config --set-str CONFIG_LOCALVERSION "-$full_postfix"         # Identifies as -yoga-amd64[cite: 13]
scripts/config --set-val CONFIG_LOCALVERSION_AUTO n                   # Cleaner versioning[cite: 13]
scripts/config --undefine CONFIG_DEBUG_INFO                           # Strip primary debug symbols[cite: 13]
scripts/config --undefine CONFIG_DEBUG_INFO_BTF                       # Disable BPF Type Format bloat[cite: 13]
scripts/config --set-val CONFIG_DEBUG_INFO_NONE y                     # Explicitly select 'None'[cite: 13]
scripts/config --disable CONFIG_DEBUG_INFO_DWARF_TOOLCHAIN_DEFAULT    # Kills bloated symbols[cite: 13]
scripts/config --disable CONFIG_DEBUG_INFO_DWARF4                     # Disables old DWARF v4[cite: 13]
scripts/config --disable CONFIG_DEBUG_INFO_DWARF5                     # Disables heavy DWARF v5[cite: 13]
scripts/config --disable CONFIG_GDB_SCRIPTS                           # No Python helpers[cite: 13]

scripts/config --disable CONFIG_DEBUG_KERNEL                          # Disables heavy diagnostic overhead[cite: 13]
scripts/config --disable CONFIG_SLUB_DEBUG                            # Speeds up memory allocation[cite: 13]
