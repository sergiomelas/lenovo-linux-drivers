			 ...
			 u32 attr, int channel, long *val)
{
	...
	/* 3. Get hardware data only for INPUT requests */
	status = acpi_evaluate_integer(data->active_handles[channel], NULL, NULL, &raw_acpi);
	if (ACPI_FAILURE(status))
		return -EIO;

	/* 4. Calculate raw RPM based on architecture */
	if (cfg->n_max > 0)
	...
	/* 5. Apply filter only for real speed readings */
...
static int yoga_fan_probe(struct platform_device *pdev)
{
	...
	/* Allocate the channel info struct (Crucial to avoid crash) */
	...
	chip_info_array[0] = info;  /* Point to the channel data, NOT chip_info */
